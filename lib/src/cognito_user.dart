...
class CognitoUser {
  ...
  Future<CognitoUserSession?> refreshSession(CognitoRefreshToken refreshToken) async {
  ...
  Future<CognitoUserSession?> getDeviceResponse() async {
    ...
    final dataAuthenticate = await client!.request('RespondToAuthChallenge', await _analyticsMetadataParamsDecorator.call(paramsResp));
  ...
  Future<CognitoUserSession?> _authenticateUserDefaultAuth(
    ...
    if (challengeName == 'NEW_PASSWORD_REQUIRED') {
      ...
      throw CognitoUserNewPasswordRequiredException(userAttributes: userAttributes, requiredAttributes: requiredAttributes);
    }
  ...
  Future<CognitoUserSession?> sendCustomChallengeAnswer(String answerChallenge, [Map<String, String>? validationData]) async {
    final challengeResponses = {
      'USERNAME': username,
    ...
    final data = await client!.request('RespondToAuthChallenge', await _analyticsMetadataParamsDecorator.call(paramsReq));
  ...
  Future<CognitoUserSession?> sendNewPasswordRequiredAnswer(String newPassword, [Map<String, String>? requiredAttributes]) async {
    final challengeResponses = {
      'USERNAME': username,
  ...
  Future<CognitoUserSession?> sendMFACode(String confirmationCode, [String mfaType = 'SMS_MFA']) async {
    ...
    final dataAuthenticate = await client!.request('RespondToAuthChallenge', await _analyticsMetadataParamsDecorator.call(paramsReq));
    ...
    final authenticationHelper = AuthenticationHelper(pool.getUserPoolId()!.split('_')[1]);
    authenticationHelper.generateHashDevice(dataAuthenticate['AuthenticationResult']['NewDeviceMetadata']['DeviceGroupKey'],
        dataAuthenticate['AuthenticationResult']['NewDeviceMetadata']['DeviceKey']);

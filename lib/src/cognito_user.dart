...
class CognitoUser {
  ...
  Future<CognitoUserSession?> getSession() async {
    ...
    if (await storage!.getItem(idTokenKey) != null) {
      final idToken = CognitoIdToken(
          await (storage!.getItem(idTokenKey) as FutureOr<String?>));
      final accessToken = CognitoAccessToken(
          await (storage!.getItem(accessTokenKey) as FutureOr<String?>));
      final refreshToken = CognitoRefreshToken(
          await (storage!.getItem(refreshTokenKey) as FutureOr<String?>));
      final clockDrift = int.parse(
          await (storage!.getItem(clockDriftKey) as FutureOr<String>));
  ...
  Future<CognitoUserSession?> refreshSession(
    ...
    if (await storage!.getItem(lastUserKey) != null) {
      username = await (storage!.getItem(lastUserKey) as FutureOr<String?>);
      final deviceKeyKey = '$keyPrefix.$username.deviceKey';
      _deviceKey = await (storage!.getItem(deviceKeyKey) as FutureOr<String?>);
  ...
  Future<void> getCachedDeviceKeyAndPassword() async {
    ...
    if (await storage!.getItem(deviceKeyKey) != null) {
      _deviceKey = await (storage!.getItem(deviceKeyKey) as FutureOr<String?>);
      _deviceGroupKey =
          await (storage!.getItem(deviceGroupKeyKey) as FutureOr<String?>);
      _randomPassword =
          await (storage!.getItem(randomPasswordKey) as FutureOr<String?>);
    }
  }
  ...
  Future<void> cacheTokens() async {
    ...
    await Future.wait([
      storage!
          .setItem(idTokenKey, _signInUserSession!.getIdToken().getJwtToken()),
      storage!.setItem(
